...
int wrapfd; // wrapped file descriptor from our termux-usb
...
////        libusb_init(&context); // initialized in main...
        //);
        //assert(!
        ////libusb_wrap_sys_device(context, (intptr_t) fd, &handle);
        libusb_wrap_sys_device(context, (intptr_t) wrapfd, &handle);
        //);
...
*/
	...
	if ((show_help) || (argc == 1) || (argc > 7)) {
		printf("usage: %s [-h] [-d] [-i] [-k] [-b file] [-l lang] [-j] [-x] [-s] [-p] [-w] <FD:automaticly passed in from termux-usb>\n", argv[0]);
        ...
        // termux passed in FD as last argument
	sscanf(argv[argc - 1], "%d", &wrapfd);
	printf("Termux passed in FD: %d\n", wrapfd);

	// Needed for termux android wrapped fd
	...
	test_device(0,0); // unused args //// VID, PID);
